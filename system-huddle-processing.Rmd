...
date_end <- as.Date(paste0(year(Sys.Date())-1,"-12-31"))
...
missing_chg_cutOff <- date_end # Should this be 2 or 3?
...
scheduling_data <- tbl(conn1, "MV_DM_PATIENT_ACCESS") %>%
  ...
  mutate(Appt.DateYear = TO_DATE(CONTACT_DATE),
         ...
         Appt.Made.DateYear = TO_DATE(APPT_MADE_DATE),
         Appt.Made.Year = year(APPT_MADE_DATE),
         Appt.Made.MonNum = month(APPT_MADE_DATE)) %>%
  ...
  mutate(Visit.Method  = ifelse(VISIT_METHOD == "IN PERSON", "IN PERSON", "TELEHEALTH"),
         New.PT3 = ifelse(LOS_NAME %LIKE% "%INITIAL%" | LOS_NAME %LIKE% "%NEW%", "New", "Established"),
