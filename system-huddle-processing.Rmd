...
scheduling_data <- access_raw_tbl %>%
  left_join(radiology_dep_mapping_tbl %>% select(DEPARTMENT_ID, CLASS),
            by = c("DEPARTMENT_ID")) %>%
  mutate(radiology_check = ifelse((CLASS == "IP_OP" & PATIENT_CLASS %LIKE% '%INPATIENT%') | (CLASS == "IP_OP" & PATIENT_CLASS %LIKE% '%EMERGENCY%'), "Exclude", "Include")) %>%
